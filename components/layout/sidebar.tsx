...
export function Sidebar({ activeItem, user }: SidebarProps) {
  ...
  return (
    <aside className="flex h-full min-h-0 flex-col overflow-hidden rounded-[28px] border border-[var(--border)] bg-[var(--surface)] p-[clamp(0.8rem,1.8vh,1.25rem)] shadow-[var(--shadow-card)]">
      <div className="rounded-[24px] bg-[linear-gradient(160deg,rgba(122,160,96,0.18),rgba(95,168,211,0.14),rgba(255,255,255,0.95))] p-[clamp(0.65rem,1.5vh,1rem)]">
        <div className="flex flex-col items-center gap-[clamp(0.55rem,1.4vh,1rem)] py-[clamp(0.2rem,0.9vh,0.5rem)] text-center">
          <div className="flex h-[clamp(2.5rem,7vh,4rem)] w-[clamp(2.5rem,7vh,4rem)] items-center justify-center rounded-[22px] border border-dashed border-[var(--sage-400)] bg-white/70 text-[10px] font-semibold uppercase tracking-[0.22em] text-[var(--ink-subtle)] shadow-sm">
            Logo
          </div>

          <h1 className="text-[clamp(1.6rem,4.4vh,2.25rem)] font-semibold tracking-[-0.06em] text-[var(--ink)]">
            VENUE
          </h1>
        </div>
      </div>
        ...
        {navigation.map((item) => {
          ...
          const content = (
            <>
              ...
              {isActive ? (
                <span className="h-[clamp(0.4rem,1vh,0.625rem)] w-[clamp(0.4rem,1vh,0.625rem)] rounded-full bg-[var(--sage-500)]" />
              ) : null}
            </>
          );
